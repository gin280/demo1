// Generated by CoffeeScript 1.9.0
(function() {
  $(document).ready(function() {
    var area, checkscroll, con1, con2, myScroll, scrollUp, showtime, showtime2, showtime3, speed;
    showtime = function() {
      var checktime, d, date, h, m, month, myDate, s, weekday, year;
      checktime = function(i) {
        if (i < 10) {
          return i = '0' + i;
        } else {
          return i;
        }
      };
      myDate = new Date();
      year = myDate.getFullYear();
      month = myDate.getMonth() + 1;
      date = myDate.getDate();
      d = myDate.getDay();
      h = myDate.getHours();
      m = myDate.getMinutes();
      s = myDate.getSeconds();
      m = checktime(m);
      s = checktime(s);
      weekday = ['星期天', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六'];
      $('.one').text(year + '年' + month + '月' + date + '日' + ' ' + weekday[d] + h + ':' + m + ':' + s);
      return setTimeout(showtime, 500);
    };
    showtime();
    showtime2 = function() {
      var curtime, endtime, lefttime;
      curtime = new Date();
      endtime = new Date('2015/2/28');
      lefttime = Math.ceil((endtime.getTime() - curtime.getTime()) / (24 * 60 * 60 * 1000));
      return $('.two').text(lefttime);
    };
    showtime2();
    showtime3 = function() {
      var curtime, d, endtime, h, lefttime, m, s;
      curtime = new Date();
      endtime = new Date('2015/2/28,12:00:00');
      lefttime = parseInt((endtime.getTime() - curtime.getTime()) / 1000);
      d = parseInt(lefttime / (24 * 60 * 60));
      h = parseInt(lefttime / (60 * 60) % 24);
      m = parseInt(lefttime / 60 % 60);
      s = parseInt(lefttime % 60);
      $('.three').text(d + '天' + h + '小时' + m + '分' + s + '秒');
      if (lefttime <= 0) {
        $('.three').text('时间到了呀');
      }
      return setTimeout(showtime3, 500);
    };
    showtime3();
    area = $('#marquee2');
    con1 = $('#con1');
    con2 = $('#con2');
    speed = 50;
    area.scrollTop(0);
    scrollUp = function() {
      if (area.scrollTop() >= con1[0].scrollHeight) {
        return area.scrollTop(0);
      } else {
        return area[0].scrollTop += 3;
      }
    };
    myScroll = setInterval(scrollUp, speed);
    $('li').mouseenter(function() {
      return clearInterval(myScroll);
    });
    $('li').mouseout(function() {
      return myScroll = setInterval(scrollUp, speed);
    });

    /*
    	iliH = 24
    	time = null
    	delay = 2000
    	area2 = $('#marquee3')
    	area2[0].innerHTML+=area2[0].innerHTML
    	startScroll = ->
    		time = setInterval(scrollUp2,speed)
    		area2[0].scrollTop++
    	scrollUp2 = ->
    		if area2[0].scrollTop % iliH is 0 
    			clearInterval(time)
    			setTimeout(startScroll,delay)
    		else
    			area2[0].scrollTop++
    			if area2[0].scrollTop >= area2[0].scrollHeight/2
    				area2[0].scrollTop = 0
    	setTimeout(startScroll,delay)
     */
    checkscroll = function() {
      var aPin, dH, lastPinH, wT;
      aPin = $(".box");
      lastPinH = aPin.last().get(0).offsetTop + Math.floor(aPin.last().height() / 2);
      wT = $(window).scrollTop();
      dH = $(document).width();
      if (lastPinH < wT + dH) {
        return true;
      } else {
        return false;
      }
    };
    return $(window).scroll(function() {
      var dataInt, index, oBox, value, _ref, _results;
      dataInt = {
        'data': [
          {
            'src': '0.jpg'
          }, {
            'src': '1.jpg'
          }, {
            'src': '2.jpg'
          }, {
            'src': '3.jpg'
          }
        ]
      };
      if (checkscroll()) {
        _ref = dataInt.data;
        _results = [];
        for (index in _ref) {
          value = _ref[index];
          oBox = '<div class="box"><div class="pic">' + '<img src="img/images/' + value.src + '">';
          _results.push($('.container').append(oBox));
        }
        return _results;
      }
    });
  });

}).call(this);
